...
/// Bluejay is a simple wrapper around CoreBluetooth that focuses on making a common usage case as
/// straight forward as possible: a single connected peripheral that the user is interacting with
/// regularly (think most personal electronics devices that have an associated iOS app: fitness
/// trackers, guitar amps, etc).
///
/// It also supports a few other niceties for simplifying usage, including automatic discovery of
/// services and characteristics as they are used, as well as supporting a background task mode
/// where the interaction with the device can be written as synchronous calls running on a
/// background thread to avoid callback pyramids of death, or heavily chained promises.
public class Bluejay: NSObject { // swiftlint:disable:this type_body_length
    ...
    /// List of weak references to objects interested in receiving notifications on Bluetooth
    ...
    /// List of weak references to objects interested in receiving notifications on services
    /// changes.
    private var serviceObservers: [WeakServiceObserver] = []

    /// List of weak references to objects interested in receiving notifications on log file
    /// changes.
    private var logObservers: [WeakLogObserver] = []

    /// Reference to a peripheral that is still connecting. If this is nil, then the peripheral
    /// should either be disconnected or connected. This is used to help determine the state of the
    /// peripheral's connection.
    private var connectingPeripheral: Peripheral?

    /// Reference to a peripheral that is connected. If this is nil, then the peripheral should
    /// either be disconnected or still connecting. This is used to help determine the state of the
    /// peripheral's connection.
    ...
    /// Allows caching and defering disconnect notifications and final clean ups when there is a
    /// disconnection while running a Bluejay background task.
    ...
    #if os(iOS)
    /// Reference to the startup, **not Bluejay**, background task used for supporting state
    /// restoration.
    private var startupBackgroundTask = UIBackgroundTaskIdentifier.invalid
    #endif

    /// True when Bluejay, **not startup**, background task is running, and helps prevent calling
    /// regular read/write/listen.
    private var isRunningBackgroundTask = false

    /// Reference to a connecting peripheral during backgrouundrestoration, this is different from
    /// the normal connecting peripheral since background restoration requires an explicit connect
    /// call to restore the connecting state, and that connect call will be blocked due to multiple
    /// connect prevention if we use the normal connecting peripheral reference.
    ...
    /// Allows checking whether Bluetooth is powered on. Also returns false if Bluejay is not
    /// started yet.
    ...
    /// please re-evaluate the bluetooth state again as it may change momentarily after it has
    /// returned true
    public var isBluetoothStateUpdateImminent: Bool {
        cbCentralManager.state == .unknown ||
    ...
    /// Allowing checking whether Bluejay will automatic reconnect after an unexpected
    /// disconnection. Default is true, and Bluejay will also always set this to true on a
    /// successful connection to a peripheral. Conversely, Bluejay will always set this to false
    /// after an explicit disconnection request.
    public private(set) var shouldAutoReconnect = true

    /// Allows checking whether Bluejay is currently scanning.
    public var isScanning: Bool {
        // Cannot rely on the manager's state for isScanning as it is not usually updated
        // immediately, and while that delay might be a more accurate representation of the current
        // state, it is almost always more useful to evaluate whether Bluejay is running a scan
        // request at the top of its queue.
        queue.isScanning
    }
    ...
    /// Warning options to use for each new connection if the options are not specified at the
    /// creation of those connections.
    public private(set) var defaultWarningOptions = WarningOptions.default
    ...
    /// Enables disconnection errors or arguments to "cancelEverything" also being broadcast to
    /// active listeners, to allow them to perform cleanup or shutdown
    /// operations.
    ///
    /// Note: Currently defaults to false, to match original behaviour, because this could be quite
    /// disruptive to code that was written assuming this isn't true.
    /// Arguably should default to true, since there are some situations (such listens in background
    /// tasks without timeouts) where this is required for correct
    /// behaviour, and it may change eventually.
    public var broadcastErrorsToListeners = false
    ...
    /// `start` call after Bluejay is intialized will then initialize the CoreBluetooth stack and is
    /// required because in cases where a state resotration is trying to restore a listen on a
    /// characteristic, a listen restorer must be available before the CoreBluetooth stack is
    /// re-initialized. This two-step startup allows you to prepare and gaurantee the setup of your
    /// listen restorer in between the initialization of Bluejay and the initialization of the
    /// CoreBluetooth stack.
    ...
    /// Starting Bluejay will initialize the CoreBluetooth stack. Simply initializing a Bluejay
    /// instance without calling this function will not initialize the CoreBluetooth stack. An
    /// explicit start call is required so that we can also support proper background restoration,
    /// where CoreBluetooth must be initialized in the AppDelegate's
    /// application(_:didFinishLaunchingWithOptions:) for both starting an iOS background task and
    /// for parsing the restore identifier.
    ///
    /// - Parameters:
    ...
    public func start(mode: StartMode = .new(.default)) {
        ...
        case .new(let startOptions):
            ...
            var managerOptions: [String: Any] =
                [CBCentralManagerOptionShowPowerAlertKey: startOptions.enableBluetoothAlert]
            ...
            case .enable(let backgroundRestoreConfig):
                ...
                debugLog(
                    "Background restoration enabled with restore identifier: \(backgroundRestoreConfig.restoreIdentifier)"
                )
            }

            if isRestoring {
                debugLog("Begin startup background task for restoring CoreBluetooth.")
                startupBackgroundTask = UIApplication.shared
                    .beginBackgroundTask(expirationHandler: {
                        self.cancelEverything(
                            error: BluejayError.startupBackgroundTaskExpired,
    ...
    /// Stops all operations and clears all states in Bluejay before returning a Core Bluetooth
    /// state that can then be used by another library or code outside of Bluejay.
    ///
    /// - Returns: Returns a CBCentralManager and possibly a CBPeripheral as well if there was one
    /// connected at the time of this call.
    /// - Warning: Will crash if Bluejay has not been instantiated properly or if Bluejay is still
    /// connecting.
    ...
    /// Check to see whether the "Uses Bluetooth LE accessories" capability is turned on in the
    /// residing Xcode project.
    private func checkBackgroundSupportForBluetooth() {
        var isSupported = false

        if let backgroundModes = Bundle.main
    ...
    /// This will cancel the current and all pending operations in the Bluejay queue. It will also
    /// disconnect by default after the queue is emptied, but you can cancel everything without
    /// disconnecting.
    ///
    /// - Parameters:
    ///  - error: Defaults to a generic `cancelled` error. Pass in a specific error if you want to
    /// deliver a specific error to all of your running and queued tasks.
    ///  - shouldDisconnect: Defaults to true, will not disconnect if set to false, but only matters
    /// if Bluejay is actually connected.
    public func cancelEverything(
        error: Error = BluejayError.cancelled,
        shouldDisconnect: Bool = true
    ) {
        debugLog(
            "Cancel everything called with error: \(error.localizedDescription), shouldDisconnect: \(shouldDisconnect)"
        )
    ...
    /// Register for notifications on Bluetooth connection events and state changes. Unregistering
    /// is not required, Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter connectionObserver: object interested in receiving Bluejay's Bluetooth
    /// connection related events.
    public func register(connectionObserver: ConnectionObserver) {
        connectionObservers = connectionObservers
            .filter { $0.weakReference != nil && $0.weakReference !== connectionObserver }
    ...
    /// Unregister for notifications on Bluetooth connection events and state changes. Unregistering
    /// is not required, Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter connectionObserver: object no longer interested in receiving Bluejay's
    /// connection related events.
    ...
    /// Register for notifications when `readRSSI` is called. Unregistering is not required, Bluejay
    /// will unregister for you if the observer is no longer in memory.
    ///
    ...
    public func register(rssiObserver: RSSIObserver) {
        rssiObservers = rssiObservers
            .filter { $0.weakReference != nil && $0.weakReference !== rssiObserver }
        rssiObservers.append(WeakRSSIObserver(weakReference: rssiObserver))
    }

    /// Unregister for notifications when `readRSSI` is called. Unregistering is not required,
    /// Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter rssiObserver: object no longer interested in receiving Bluejay's `readRSSI`
    /// response.
    ...
    /// Register for notifications when a connected peripheral's services change. Unregistering is
    /// not required, Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter serviceObserver: object interested in receiving the connected peripheral's did
    /// modify services event.
    public func register(serviceObserver: ServiceObserver) {
        serviceObservers = serviceObservers
            .filter { $0.weakReference != nil && $0.weakReference !== serviceObserver }
        serviceObservers.append(WeakServiceObserver(weakReference: serviceObserver))
    }

    /// Unregister for notifications when a connected peripheral's services change. Unregistering is
    /// not required, Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter serviceObserver: object no longer interested in receiving the connected
    /// peripheral's did modify services event.
    public func unregister(serviceObserver: ServiceObserver) {
        serviceObservers = serviceObservers
            .filter { $0.weakReference != nil && $0.weakReference !== serviceObserver }
    }

    /// Register for notifications when debug logs occur inside the library. Unregistering is not
    /// required, Bluejay will unregister for you if the observer is no longer in memory. If no log
    ...
    public func register(logObserver: LogObserver) {
        logObservers = logObservers
            .filter { $0.weakReference != nil && $0.weakReference !== logObserver }
        logObservers.append(WeakLogObserver(weakReference: logObserver))
    }

    /// Unregister for notifications when ebug logs occur inside the library Unregistering is not
    /// required, Bluejay will unregister for you if the observer is no longer in memory.
    ///
    /// - Parameter logObserver: object no longer interested in notifications when the log file is
    /// updated.
    ...
    /// Register a single disconnection handler for giving it a final say on what to do at the end
    /// of a disconnection, as well as evaluate and control Bluejay's auto-reconnect behaviour.
    ///
    /// - Parameter handler: object interested in becoming Bluejay's optional but most featureful
    ...
    ///
    /// - Warning: Setting `serviceIdentifiers` to `nil` will result in picking up all available
    /// Bluetooth peripherals in the vicinity, **but is not recommended by Apple**. It may cause
    /// battery and cpu issues on prolonged scanning, and **it also doesn't work in the
    /// background**. If you need to scan for all Bluetooth devices, we recommend making use of the
    /// `duration` parameter to stop the scan after 5 ~ 10 seconds to avoid scanning indefinitely
    /// and overloading the hardware.
    ///
    /// - Parameters:
    ///   - duration: Stops the scan when the duration in seconds is reached. Defaults to zero
    /// (indefinite).
    ///   - allowDuplicates: Determines whether a previously scanned peripheral is allowed to be
    /// discovered again.
    ///   - throttleRSSIDelta: Throttles discoveries by ignoring insignificant changes to RSSI.
    ///   - serviceIdentifiers: Specifies what visible services the peripherals must have in order
    /// to be discovered.
    ///   - discovery: Called whenever a specified peripheral has been discovered.
    ///   - expired: Called whenever a previously discovered peripheral has not been seen again for
    /// a while, and Bluejay is predicting that it may no longer be in range. (Only for a scan with
    ...
    /// Attempt to connect directly to a known peripheral. The call will fail if Bluetooth is not
    /// available, or if Bluejay is already connected. Making a connection request while Bluejay is
    /// scanning will also cause Bluejay to stop the current scan for you behind the scene prior to
    /// fulfilling your connection request.
    ///
    /// - Parameters:
    ///    - peripheralIdentifier: The peripheral to connect to.
    ///    - timeout: Specify how long the connection time out should be.
    ///    - warningOptions: Optional connection warning options, if not specified, Bluejay's
    ...
    public func connect(
        ...
        if let cbPeripheral = cbCentralManager
            .retrievePeripherals(withIdentifiers: [peripheralIdentifier.uuid]).first
        {
    ...
    ///
    /// - Attention: If you are going to use the completion block, be careful on how you orchestrate
    /// and organize multiple disconnection callbacks if you are also using a `DisconnectHandler`.
    ///
    /// - Parameters:
    ///    - immediate: If true, the disconnect will not be enqueued and will cancel everything in
    /// the queue immediately then disconnect. If false, the disconnect will wait until everything
    /// in the queue is finished.
    ///    - completion: Called when the disconnect request is fully completed.
    public func disconnect(
        immediate: Bool = false,
        completion: ((DisconnectionResult) -> Void)? = nil
    ) {
        Dispatch.dispatchPrecondition(condition: .onQueue(.main))
        ...
        if isConnecting || isConnected {
            ...
            if immediate {
                ...
                if let peripheral = connectingPeripheral?.cbPeripheral ?? connectedPeripheral?
                    .cbPeripheral
                {
    ...
    ///    - completion: Called with the result of the attempt to read from the specified
    /// characteristic.
    public func read<R: Receivable>(
        ...
        if let peripheral = connectedPeripheral {
            ...
            debugLog(
                "Cannot request read on \(characteristicIdentifier.description): \(BluejayError.notConnected.localizedDescription)"
            )
            completion(.failure(BluejayError.notConnected))
        }
    }
    ...
    ///    - type: Write type.
    ///    - completion: Called with the result of the attempt to write to the specified
    /// characteristic.
    public func write<S: Sendable>(
        ...
        if let peripheral = connectedPeripheral {
            ...
            debugLog(
                "Cannot request write on \(characteristicIdentifier.description): \(BluejayError.notConnected.localizedDescription)"
            )
            completion(.failure(BluejayError.notConnected))
        }
    }
    ...
    ///   - completion: Called with the result of the attempt to listen for notifications on the
    ...
    public func listen<R: Receivable>(
        ...
        if let peripheral = connectedPeripheral {
            peripheral.listen(
                to: characteristicIdentifier,
                multipleListenOption: option,
                completion: completion
            )
        } else {
            debugLog(
                "Cannot request listen on \(characteristicIdentifier.description): \(BluejayError.notConnected.localizedDescription)"
            )
            completion(.failure(BluejayError.notConnected))
        }
    }
    ...
    ///    - completion: Called with the result of the attempt to stop listening to the specified
    /// characteristic.
    public func endListen(
        ...
        if let peripheral = connectedPeripheral {
            ...
            debugLog(
                "Cannot request end listen on \(characteristicIdentifier.description): \(BluejayError.notConnected.localizedDescription)"
            )
            completion?(.failure(BluejayError.notConnected))
        }
    }
    ...
    ///
    /// - Warning: Will throw if called while a Bluejay background task is running, or if not
    /// connected.
    ...
    /// One of the three ways to run a background task using a synchronous interface to the
    /// Bluetooth peripheral. This is the simplest one as the background task will not return any
    /// typed values back to the completion block on finishing the background task, except for
    /// thrown errors, and it also doesn't provide an input for an object that might need thread
    /// safe access.
    ///
    ...
    ///   - completionOnMainThread: A closure called on the main thread when the background task has
    ...
    /// One of the three ways to run a background task using a synchronous interface to the
    /// Bluetooth peripheral. This one allows the background task to potentially return a typed
    /// value back to the completion block on finishing the background task successfully.
    ///
    /// - Warning: Be careful not to access anything that is not thread safe inside background task.
    ///
    /// - Parameters:
    ...
    /// One of the three ways to run a background task using a synchronous interface to the
    /// Bluetooth peripheral. This one allows the background task to potentially return a typed
    /// value back to the completion block on finishing the background task successfully, as well as
    /// supplying an object for thread safe access inside the background task.
    ///
    /// - Warning: Be careful not to access anything that is not thread safe inside background task.
    ///
    /// - Parameters:
    ...
    ///
    /// - Parameter sendables: An array of Sendables whose Data should be appended in the order of
    /// the given array.
    ///
    /// - Returns: The resulting data of all the Sendables combined in the order of the passed in
    /// array.
    ...
    private func endStartupBackgroundTask() {
        ...
        if startupBackgroundTask != UIBackgroundTaskIdentifier.invalid {
            ...
            UIApplication.shared
                .endBackgroundTask(convertToUIBackgroundTaskIdentifier(
        ...
        // Startup background task should always and only be ended after background restoration is
        // completed.
...
extension Bluejay: CBCentralManagerDelegate {
    ...
    ///
    /// A manual connect attempt is required (Bluejay will do this for you), as CoreBluetooth does
    /// not have a connection in-progress, nor does it issue a connect for you even when it is
    /// restoring into a connecting state.
    ///
    /// - Parameter peripheral: the connecting peripheral restored.
    private func restoreConnecting(peripheral: Peripheral) {
        guard let backgroundRestorer = backgroundRestorer else {
        ...
        connect(peripheral.identifier, timeout: .seconds(15)) { result in
            ...
            case .success(let peripheral):
                ...
                let backgroundRestoreCompletion = backgroundRestorer
                    .didRestoreConnection(to: peripheral)
            ...
            case .failure(let error):
                self
                    .debugLog(
                        "Did fail to to restore connection with error: \(error.localizedDescription)"
                    )
    ...
    ///
    /// No further connection related actions are required, simply notify the background restorer as
    /// well as the connection observers, as interaction with the peripheral is now possible.
    ///
    ...
    private func restoreConnected(peripheral: Peripheral) {
        ...
        let backgroundRestoreCompletion = backgroundRestorer
            .didRestoreConnection(to: peripheral.identifier)
    ...
    ///
    /// There is currently no known way to recreate nor to test this scenario. For now, we believe
    /// centralManager(_:didDisconnectPeripheral:error:) will not be called in this case, so simply
    /// notify failure to restore connection and assume CoreBluetooth will clean up and discard the
    /// unused and disconnecting peripheral as long as we don't hold a reference to the
    /// disconnecting `CBPeripheral`.
    ///
    ...
    /// centralManager(_:didDisconnectPeripheral:error:) will not be called in this case, so simply
    /// notify failure to restore connection and assume CoreBluetooth will clean up and discard the
    /// unused and disconnected peripheral as long as we don't hold a reference to the disconnected
    /// `CBPeripheral`.
    ///
    ...
    /// - If Bluetooth is available for the first time and the app is already connected, then this
    /// is a state restoration event. Try listen restoration if possible.
    /// - If Bluetooth is turned off, cancel everything with the `bluetoothUnavailable` error and
    /// disconnect.
    ...
    public func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        case .poweredOn:
            if isRestoring {
                // Clear everything that might be in the queue before completing background
                // restorations.
                cancelEverything(
                    error: BluejayError.backgroundRestorationInProgress,
                    shouldDisconnect: false
                )

                // Safe to start the queue once it is emptied to allow background restorations, as
                // well as subsequent user requests to complete.
                queue.start()
    ...
    /// If Core Bluetooth will restore state, update Bluejay's internal states to match the states
    /// of the Core Bluetooth stack by assigning the peripheral to `connectingPeripheral` or
    /// `connectedPeripheral`, or niling them out, depending on what the restored `CBPeripheral`
    /// state is.
    public func centralManager(_ central: CBCentralManager, willRestoreState dict: [String: Any]) {
        debugLog("Central manager will restore state.")

        guard let peripherals = dict[CBCentralManagerRestoredStatePeripheralsKey] as? [
            CBPeripheral
        ],
        ...
        case .disconnected:
            precondition(
                connectingPeripheral == nil && connectedPeripheral == nil,
    ...
    /// When connected, update Bluejay's states by updating the values for `connectingPeripheral`,
    /// `connectedPeripheral`, and `shouldAutoReconnect`. Also, make sure to broadcast the event to
    /// observers, and notify the queue so that the current operation in-flight can process this
    /// event and get a chance to finish.
    public func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        debugLog(
            "Central manager did connect to: \(peripheral.name ?? peripheral.identifier.uuidString)"
        )
        ...
        precondition(
            connectedPeripheral != nil,
            "Connected peripheral is assigned a nil value despite Bluejay has successfully finished a connection."
        )
    ...
    /// Handle a disconnection event from Core Bluetooth by figuring out what kind of disconnection
    /// it is (planned or unplanned), and updating Bluejay's internal state and sending
    ...
    public func centralManager(
        ...
        if let errorMessage = errorString {
            debugLog(
                "Central manager did disconnect from \(peripheralString) with error: \(errorMessage)"
            )
        } else {
        ...
        if wasRestoringConnectingPeripheral {
            debugLog(
                "Peripheral state was connecting during background restoration before centralManager(_:didDisconnectPeripheral:error:)."
            )
        } else if wasRestoringDisconnectingPeripheral {
            debugLog(
                "Peripheral state was disconnecting during background restoration before centralManager(_:didDisconnectPeripheral:error:)."
            )
        } else if wasRestoringDisconnectedPeripheral {
            debugLog(
                "Peripheral state was disconnected during background restoration before centralManager(_:didDisconnectPeripheral:error:)."
            )
        } else if wasConnecting {
            debugLog(
                "Peripheral was still connecting before centralManager(_:didDisconnectPeripheral:error:)."
            )
        } else if wasConnected {
            debugLog(
                "Peripheral was connected before centralManager(_:didDisconnectPeripheral:error:)."
            )
        }
        ...
        if (!isDisconnecting && !queue.isRunningQueuedDisconnection) ||
            wasRestoringConnectingPeripheral
        {
        ...
        disconnectCleanUp = { [weak self] in
            ...
            if wasConnecting || weakSelf.queue.isRunningQueuedDisconnection {
                ...
                if wasConnecting {
                    weakSelf
                        .debugLog(
                            "Disconnect clean up: delivering expected disconnected event back to the pending connection in the queue..."
                        )
                    ...
                    if let connection = weakSelf.queue.first as? Connection {
                        ...
                        } else if case .stopping(let error) = connection.state {
                    ...
                    weakSelf
                        .debugLog(
                            "Disconnect clean up: delivering expected disconnected event back to the queued disconnection in the queue..."
                        )
                }

                // Allow the Connection or Disconnection operation to finish its cancellation,
                // trigger its callback, and continue cancelling any remaining operations in the
                // queue.
                weakSelf.queue.process(
                    event: .didDisconnectPeripheral(disconnectedPeripheral),
                    error: nil
                )
            } else if wasConnected {
                precondition(
                    weakSelf.queue.isEmpty,
                    "Queue should be emptied before notifying and invoking all disconnect observers and callbacks."
                )
            }
            ...
            if wasRestoringConnectingPeripheral {
                weakSelf
                    .debugLog(
                        "Disconnect clean up: disconnected while restoring a connecting peripheral, will not auto-reconnect."
                    )
                ...
                weakSelf
                    .debugLog(
                        "Disconnect clean up: should auto-reconnect: \(weakSelf.shouldAutoReconnect)"
                    )

                if let disconnectHandler = weakSelf.disconnectHandler {
                    weakSelf.debugLog("Disconnect clean up: calling the disconnect handler.")
                    switch disconnectHandler.didDisconnect(
                        from: disconnectedPeripheral.identifier,
                        with: error,
                        willReconnect: weakSelf.shouldAutoReconnect
                    ) {
                    case .noChange:
                    ...
                    case .change(let autoReconnect):
                        ...
                        weakSelf
                            .debugLog(
                                "Disconnect handler changing auto-reconnect to: \(weakSelf.shouldAutoReconnect)"
                            )
                    }
                }
                ...
                if isExpectedDisconnect {
                    weakSelf
                        .debugLog(
                            "Disconnect clean up: calling the explicit disconnect callback if it is provided."
                        )
                ...
                if wasConnecting {
                    guard let connectingError = connectingError else {
                        preconditionFailure(
                            "Missing connecting error at the end of a disconnect clean up after cancelling a pending connection."
                        )
                    }

                    weakSelf
                        .debugLog(
                            "Disconnect clean up: calling the connecting callback if it is provided."
                        )
                    weakSelf.connectingCallback?(.failure(connectingError))
                    weakSelf.connectingCallback = nil
                }
                ...
                if weakSelf.shouldAutoReconnect {
                    weakSelf
                        .debugLog(
                            "Disconnect clean up: issuing reconnect to: \(peripheral.name ?? peripheral.identifier.uuidString)"
                        )
                    weakSelf.connect(
    ...
    /// This mostly happens when either the Bluetooth device or the Core Bluetooth stack somehow
    /// only partially completes the negotiation of a connection. For simplicity, Bluejay is
    /// currently treating this as a disconnection event, so it can perform all the same clean up
    /// logic.
    public func centralManager(
        _ central: CBCentralManager,
    ...
    public func centralManager(
        ...
        // let peripheralString = advertisementData[CBAdvertisementDataLocalNameKey] ??
        // peripheral.identifier.uuidString
        // log("Did discover: \(peripheralString)")

        queue.process(
            event: .didDiscoverPeripheral(peripheral, advertisementData, RSSI),
            error: nil
        )
    }
}
...
extension Bluejay: PeripheralDelegate {
    ...
    func isReading(characteristic: CharacteristicIdentifier) -> Bool {
        queue.isReading(characteristic: characteristic)
    }

    func willEndListen(on characteristic: CharacteristicIdentifier) -> Bool {
        queue.willEndListen(on: characteristic)
    }
    ...
    func receivedUnhandledListen(
        ...
        let listenRestoreAction = listenRestorer.didReceiveUnhandledListen(
        ...
        case .promiseRestoration:
            debugLog(
                "Promised restoration for listen on \(characteristic.description) for \(peripheral.identifier.description)"
            )
        case .stopListen:
            debugLog(
                "End listen requested for listen on \(characteristic.description) for \(peripheral.identifier.description)"
            )
    ...
    func didReadRSSI(from peripheral: Peripheral, RSSI: NSNumber, error: Error?) {
        for observer in rssiObservers {
            observer.weakReference?.didReadRSSI(
                from: peripheral.identifier,
                RSSI: RSSI,
                error: error
            )
...
private func convertToUIBackgroundTaskIdentifier(_ input: Int) -> UIBackgroundTaskIdentifier {
    UIBackgroundTaskIdentifier(rawValue: input)
}
#endif
